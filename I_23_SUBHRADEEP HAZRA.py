...
def createfile(name,lst):
    with open('{path}/{name}','a',newline='')as x:
        script= csv.writer(x)
        script.writerow(lst)
        print(" The Directory : {name} has been duly Updated !!!!! ")
...
#batch assigner
def get_batch():
    with open('C:/ProgrammingProject2022_Database/batchrecords.csv','r') as x:
        reader=csv.reader(x)
...
def remove(string):
    with open('C:/ProgrammingProject2022_Database/studentrecords.csv','r+',newline='') as x:
        script=csv.reader(x)
...
def course_graph():
    ...
    count=0
    with open('{path}/courserecords.csv','r')as x:
        script= csv.reader(x)
        rows=[row for row in script]
        req=[]
        for i in range(len(rows)):
...
def batch_graph(arg):
    with open('{path}/batchrecords.csv','r') as x:
        reader=csv.reader(x)
        req=''
        rows=[row for row in reader]
        for i in range(len(rows)):
...
try:
    os.makedirs('{path}/StudentReportCards')
    message=True
except:
    message=False
...
while message:
    ...
    with open('{path}/courserecords.csv','a',newline='')as x:
        script= csv.writer(x)
    ...
    with open('{path}/departmentrecords.csv','a',newline='')as x:
        script= csv.writer(x)
...
#id-creations
    ...
    if duplicate('studentrecords.csv',student_id,0):
        ...
        print("Report card for the student can be found here : {path}/StudentReportCards/{student_id}_{name}.txt")
    else:
        ...
        with open("{path}/StudentReportCards/{student_id}_{''.join(name.split())}.txt",'w') as x:

            x.writelines([' Student Name : {name} \n',
                          ' Class Roll-number: {roll} \n',
                          ' Stream : {stream} \n',
                          ' Student ID : {student_id}\n',
                          '\n\n Marks obtained in the Following subjects are\n ','->-'*15,
                          '\n Mathematics :- {marks_lst[0]} \n'])
            if stream.lower()=='cse' or stream.lower()=='cseai' or stream.lower()=='cseaiml' or stream.lower()=='cseiot' or stream.lower()=='csbs':
                        x.writelines([' Physics :- {marks_lst[1]} \n'])
            elif stream.lower()=='it' or stream.lower()=='ece' or stream.lower()=='me':
                        x.writelines([' Chemistry :- {marks_lst[2]} \n'])
            x.writelines([' Electrical :- {marks_lst[3]} \n',
                          ' Mechanics :- {marks_lst[4]} \n'
                          ' PythonProgramming :- {marks_lst[5]} \n',
                          ' Biology :- {marks_lst[6]} \n'])
            x.write('\n')
            x.write('Total marks out of 600 is : {total_marks} and Percentage scored is : {percent(total_marks)}%\n')
            x.write(grade(total_marks/(count(marks_lst)-1)))
            x.write('\n')
        createfile('studentrecords.csv',[student_id,name,roll,batch_id])
        print("You can find your report card here : {path}/StudentReportCards/{student_id}_{''.join(name.split())}.txt")
        openpath="{path}/StudentReportCards/{student_id}_{''.join(name.split())}.txt"
        ...
        if ask.lower()=='n':
            if duplicate('batchrecords.csv',batch_id,0):
                with open('{path}/batchrecords.csv','r+',newline='') as x:
                    script=csv.reader(x)
                    rows=[row for row in script]
                    for i in rows:
                        if batch_id==i[0]:
                            rows[rows.index(i)][4]+=':{student_id}'
                    x.seek(0)
                    x.truncate()
                    writer=csv.writer(x)
                    writer.writerows(rows)
            ...
            with open('{path}/courserecords.csv','r+',newline='') as x:
                script=csv.reader(x)
                rows=[row for row in script]
                for i in range(len(rows)):
                    if i==0:
                        ...
                        try:
                            rows[i][2]+='{student_id}:{marks_lst[i-1]}-'
                        except:
                            rows[i].append('{student_id}:{marks_lst[i-1]}-')
                x.seek(0)
                x.truncate()
                writer=csv.writer(x)
                writer.writerows(rows)
...
try:
    with open('{path}/departmentrecords.csv','r+',newline='') as x:
        script=csv.reader(x)
        rows=[row for row in script]
        lst=get_batch()
        for i in lst:
                ...
                if i[0:-2]==j[0]:
                    try:
                        if i in j[2]:
                            ...
                            rows[rows.index(j)][2]+='{i}:'
                    except:
                        rows[rows.index(j)].append('{i}:')
                    break
        x.seek(0)
        x.truncate()
        writer=csv.writer(x)
...
while True:
    if batch_id in batch:
        ...
        print('No Batch with Batch ID : {batch_id} exists in the directory')
...
while True:
    ...
    if ask2.lower()=='y':
            loading_screen()
            openpath="{path}/batchrecords.csv"
            subprocess.run(['start',openpath], shell=True)
            print("\n ")
            print("\n ")
            loading_screen()
            openpath="{path}/courserecords.csv"
            subprocess.run(['start',openpath], shell=True)
            print("\n ")
            print("\n ")
            loading_screen()
            openpath="{path}/departmentrecords.csv"
            subprocess.run(['start',openpath], shell=True)
            print("\n ")
            print("\n ")
            loading_screen()
            openpath="{path}/studentrecords.csv"
            subprocess.run(['start',openpath], shell=True)
            print("\n ")
            print("\n ")
            loading_screen()
            openpath="{path}/examresults.csv"
            subprocess.run(['start',openpath], shell=True)
